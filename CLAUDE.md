...
| 项目 | 值 |
|------|-----|
| **前端 dev 地址** | 以本项目 `src-tauri/tauri.conf.json` 的 `devUrl` 为准（等于 `vite.config.ts` 的 `server.port`，`strictPort: true` 恒定不漂移）。**各子项目端口不同**（本模板默认 1420，衍生项目如 1441 / 1445 / 1448 / 1451 各异），**切勿写死 1420** |
