---
name: sync-from-framework
description: |
  原框架到子项目的更新拉取员，2-way diff 比对框架与本项目，把框架里更新的 skills / commands / 配置 / 依赖逐条报给用户确认。

  触发场景：
  - 框架有更新，需要拉取到本子项目
  - 需要对比框架与子项目的 skills / commands 差异
  - 子项目的框架副本已落后需要同步

  触发词：同步框架、framework-sync、sync-from-framework、拉取框架更新、框架副本落后、上游同步
---
